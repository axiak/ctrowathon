...
(function ($, window) {
  var unitMaps = [
    ["meters", 1, "meter"],
    ["miles", 0.0006213711, "mile"],
    ["football fields", 0.00364537766, "football field"],
    ["boneless skinless chicken breasts", 6.92125, "boneless skinless chicken breast"]
  ];
  ...
  var utils = {
    ...
    "createUnitDom": function (distance) {
      ...
      for (var i = 0; i < unitMaps.length; ++i) {
        var intval = Math.round(distance * unitMaps[i][1]);
        var currentDistance = this.addCommas(intval);
        var child = $("<span>" + currentDistance + " " + unitMaps[i][intval == 1 ? 2 : 0] + "</span>")
  ...
  $(function () {
    ...
    setInterval(function () {
      ...
      distance += (-Math.log(Math.random()) / .1);
            utils.createUnitDom(distance);
            utils.updateClock(time);
